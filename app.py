...
# --- 3. SECURE AI SETUP ---
try:
    # This grabs the key you paste into the Secrets box
    if "GOOGLE_API_KEY" in st.secrets:
        API_KEY = st.secrets["GOOGLE_API_KEY"]
        genai.configure(api_key=API_KEY)
    else:
        st.error("⚠️ API Key not found. Please add GOOGLE_API_KEY to Streamlit Secrets.")
        st.stop()
...
def get_working_model():
    ...
    try:
        ...
        return genai.GenerativeModel(preferences[1]) # Default to Flash for speed
    except:
